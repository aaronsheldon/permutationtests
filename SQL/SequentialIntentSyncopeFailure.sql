 ...
 */
ALTER PROCEDURE SequentialIntentSyncopeFailure AS
    ...
    (
      ...
      SELECT G.Id AS Id, MIN(G.EventDate) AS EventDate
        ...
        (
          ...
          /* Only use withdraw for study completion and ineligibility */
          SELECT F.PatientId, F.WithdrawDate
            FROM FollowUp AS F            
            WHERE F.WithdrawComments IS NULL
              OR UPPER(F.WithdrawComments) LIKE '%PACEMAKER%'
              OR UPPER(F.WithdrawComments) LIKE '%EPILEPSY%'
              OR UPPER(F.WithdrawComments) LIKE '%CANCER%'
              OR UPPER(F.WithdrawComments) LIKE '%PREGNAN%'
        ) AS G
